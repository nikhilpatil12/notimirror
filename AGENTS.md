...
**Key ANCS details:**
- Data Source response layout: `[CommandID 1B][UID 4B LE][attributes...]`. The UID is at byte offset 1, not 0.
- NS/DS subscriptions must be sequential with 200ms delay between them. BLE allows only one descriptor write at a time; simultaneous subscription attempts cause DS to fail silently.

**BLE packet handling (CRITICAL):**
- **Buffer reuse:** Android BLE stack reuses `ByteArray` buffers across callbacks. `BleConnectionManager.onCharacteristicChanged()` MUST copy the byte array IMMEDIATELY with `value.copyOf()` before any async processing. Without this, data corruption occurs (e.g., "Nikhil Patil" becomes "Nikhil P6062" as the buffer gets overwritten with date data).
- **Duplicate callbacks:** Android delivers identical `onCharacteristicChanged` callbacks to multiple threads. `BleConnectionManager` deduplicates synchronously in the callback using packet hashes per characteristic UUID, clearing every 500ms. This prevents duplicate processing at the lowest level before data reaches `AncsClient`.
- Industry standard pattern: Copy buffers + deduplicate synchronously in BLE callback, BEFORE launching coroutines or emitting events.
